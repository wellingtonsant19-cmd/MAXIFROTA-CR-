...
def process_files(csv_bytes, xlsx_bytes):
    ...
    # ── 9. ABAS DE RESUMO ────────────────────────────────────────────
    print("\n📊 Gerando abas de resumo ...")

    CORES = ['#1F4E79','#2E75B6','#70AD47','#FFC000','#FF6B35',
             '#C00000','#7030A0','#00B0F0','#92D050','#FF0000',
             '#595959','#A9D18E','#FFE699','#BDD7EE','#D6DCE4']

    MES_NOME = {1:'Janeiro',2:'Fevereiro',3:'Março',4:'Abril',5:'Maio',
                6:'Junho',7:'Julho',8:'Agosto',9:'Setembro',10:'Outubro',
                11:'Novembro',12:'Dezembro'}

    def write_resumo(df_src, sname, date_col, titulo, chart_titulo):
        ws = wb.add_worksheet(sname)

        # Formatos exclusivos da aba de resumo
        F_T  = wb.add_format({'bold':True,'font_size':14,'font_name':'Arial',
                               'font_color':'#00205B','valign':'vcenter'})
        F_S  = wb.add_format({'italic':True,'font_size':10,'font_name':'Arial',
                               'font_color':'#666666'})
        F_CH = wb.add_format({'bold':True,'bg_color':'#00205B','font_color':'#FFFFFF',
                               'border':1,'align':'center','valign':'vcenter',
                               'text_wrap':True,'font_name':'Arial','font_size':10})
        F_RH = wb.add_format({'bold':True,'bg_color':'#D6E4F0','font_color':'#00205B',
                               'border':1,'align':'center','font_name':'Arial','font_size':10})
        F_V  = wb.add_format({'num_format':'#,##0.00','border':1,
                               'align':'right','font_name':'Arial','font_size':10})
        F_V0 = wb.add_format({'num_format':'#,##0.00','border':1,
                               'font_color':'#BBBBBB','align':'right',
                               'font_name':'Arial','font_size':10})
        F_TR = wb.add_format({'bold':True,'bg_color':'#FFF2CC','border':1,
                               'num_format':'#,##0.00','align':'right',
                               'font_name':'Arial','font_size':10})
        F_TL = wb.add_format({'bold':True,'bg_color':'#FFF2CC','border':1,
                               'align':'center','font_name':'Arial','font_size':10})
        ...
        df = df[df[date_col].notna() &
                (df[date_col].dt.month == mes_atual) &
                (df[date_col].dt.year  == ano_atual)]

        if df.empty:
            ws.merge_range(0,0,0,5,
                f'Sem dados para {MES_NOME[mes_atual]}/{ano_atual}', F_T)
            ws.set_column(0,5,30)
            return

        df = df.copy()
        df['_dia']   = df[date_col].dt.day
        df['_grupo'] = df['GRUPO'].apply(
            lambda v: str(v).strip()
            if str(v).strip() not in ('','nan','None','REVISAR')
            else 'SEM GRUPO')
        df['_vl'] = pd.to_numeric(df['VL SALDO'], errors='coerce').fillna(0)

        pivot = df.groupby(['_dia','_grupo'])['_vl'].sum().unstack(fill_value=0)
        ...
        ws.merge_range(0, 0, 0, n_grp+2,
            f'📊  {titulo} — {MES_NOME[mes_atual]}/{ano_atual}', F_T)
        ws.merge_range(1, 0, 1, n_grp+2,
            f'Valores em R$  •  {n_dias} dia(s) com movimento  '
            f'•  {len(df):,} títulos  '
            f'•  Total: R$ {df["_vl"].sum():,.2f}', F_S)

        # Linha de cabeçalho da tabela
        HR = 3
        ws.set_row(HR, 28)
        ws.write(HR, 0, 'DIA',       F_CH)
        ws.write(HR, 1, 'DATA',      F_CH)
        for ci, grp in enumerate(grupos):
            ws.write(HR, ci+2, grp,  F_CH)
        ws.write(HR, n_grp+2, 'TOTAL DIA', F_CH)

        # Dados linha a linha
        DS = HR + 1
        for ri, dia in enumerate(dias):
            xr = DS + ri
            ws.write(xr, 0, int(dia),
                     f'{int(dia):02d}/{mes_atual:02d}/{ano_atual}')
            ws.write(xr, 0, int(dia), F_RH)
            ws.write(xr, 1, f'{int(dia):02d}/{mes_atual:02d}/{ano_atual}', F_RH)
            td = 0.0
            for ci, grp in enumerate(grupos):
                v = float(pivot.loc[dia, grp]); td += v
                ws.write_number(xr, ci+2, v, F_V if v > 0 else F_V0)
            ws.write_number(xr, n_grp+2, td, F_TR)
        ...
        gt = 0.0
        for ci, grp in enumerate(grupos):
            ct = float(pivot[grp].sum()); gt += ct
            ws.write_number(TR, ci+2, ct, F_TR)
        ws.write_number(TR, n_grp+2, gt, F_TR)

        # Larguras de coluna
        ws.set_column(0, 0, 7)
        ws.set_column(1, 1, 13)
        for ci in range(n_grp):
            ws.set_column(ci+2, ci+2, max(18, len(grupos[ci])+3))
        ws.set_column(n_grp+2, n_grp+2, 14)
        ...
        for ci, grp in enumerate(grupos):
            chart.add_series({
                'name':       grp,
                'categories': [sname, DS, 0, DS+n_dias-1, 0],
                'values':     [sname, DS, ci+2, DS+n_dias-1, ci+2],
                'fill':       {'color': CORES[ci % len(CORES)]},
                'border':     {'color': '#FFFFFF', 'width': 0.75},
            })
        chart.set_title({
            'name': chart_titulo,
            'name_font': {'bold':True,'size':12,'color':'#00205B','name':'Arial'}
        })
        chart.set_x_axis({
            'name': 'Dia do Mês',
            'name_font': {'bold':True,'name':'Arial','size':10},
        ...
        chart.set_chartarea({'border':{'color':'#CCCCCC'},'fill':{'color':'#FAFAFA'}})
        chart.set_size({'width': min(960, 320+n_dias*28), 'height': 420})
        ws.insert_chart(TR+3, 0, chart, {'x_offset':5,'y_offset':5})
        print(f"   ✓ {sname}: {n_dias} dias · {n_grp} grupos · "
              f"R$ {gt:,.2f} total")

    # Preparar A VENCER: usar PREV PAGTO como data de previsão
    df_av_resumo = df_av.copy()
    df_av_resumo['PREV PAGTO'] = df_av_resumo['PREV PAGTO'].apply(
        lambda v: pd.NaT
        if str(v).strip() in ('REVISAR','','nan','None') else v)
    df_av_resumo['PREV PAGTO'] = pd.to_datetime(
        df_av_resumo['PREV PAGTO'], errors='coerce')

    # Resumos aparecem primeiro nas abas (ordem da esquerda para direita)
    write_resumo(df_av_resumo, 'RESUMO A VENCER', 'PREV PAGTO',
                 'Previsão de Recebimento por Grupo',
                 'Previsão de Pagamentos por Dia e Grupo')
    write_resumo(df_vd, 'RESUMO VENCIDOS', 'VENCIMENTO',
                 'Títulos Vencidos por Grupo',
                 'Valores Vencidos por Dia e Grupo')
